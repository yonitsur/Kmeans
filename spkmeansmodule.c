...
/* input: PyObject matrix, matrix A, matrix B.
if(B!=NULL): set matrix to B with the diagonal of A as the  
             first row --> matrix:([(N+1)*N]). | k == N,  A,B:([N*N])
else: set matrix to A --> matrix:([N*K]). | A:([N*K]) */
void set_matrix(PyObject* matrix, double **A, double **B, int N, int k){
    int i,j;
    PyObject* rows;
    ...
    for(i=0; i<N+(B!=NULL); i++){
        ...
        PyList_SetItem(matrix, i, Py_BuildValue("O", rows));
    }
}
